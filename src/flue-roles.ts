...
export function validateConfiguredFlueRoles(config: ProjectConfig, availableRoles: string[]): void {
  const missingRequiredFields = missingRequiredProjectRoleFields(config);
  if (missingRequiredFields.length > 0) {
    throw new Error(`Configured Flue roles are missing required fields: ${missingRequiredFields.join(", ")}`);
  }

  const available = new Set(availableRoles);
  const missing = configuredFlueRoleReferences(config).filter((reference) => !available.has(reference.role));
  ...
  const missingLines = [...byRole.entries()].map(([role, fields]) => `- ${role}: ${fields.join(", ")}`);
